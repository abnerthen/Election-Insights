...
// ── 1. Grid View (Coordinate 15x15 or flow fallback) ───────────────────────
...
export function ConstituencyRowView({ constituencies }: ConstituencyMapProps) {
  ...
  // Sort constituencies in each region by their code first
  Object.keys(regionsMap).forEach(regionName => {
    regionsMap[regionName].sort((a, b) => compareCodes(a.code, b.code));
  });

  // Sort regions based on the minimum constituency code in that region
  const sortedRegions = Object.keys(regionsMap).sort((a, b) => {
    const minA = regionsMap[a][0]?.code || "";
    const minB = regionsMap[b][0]?.code || "";
    return compareCodes(minA, minB);
  });
  ...
  return (
    <div className="w-full">
      ...
      <div className="space-y-1 max-w-[800px] mx-auto p-4 bg-card border border-border rounded-lg shadow-xl">
